...
describe("PowerApps Basic Tests", function () {
  ...
  it("GET /WhoAmI", async function () {
    ...
    expect(json.OrganizationId).to.match(
  ...
  it("DeserializeJSON", async function () {
    ...
    let result: any = dj.deserializeFromJson(
      JSON.parse('{"id":1,"make":"Ford","model":"Escape","year":2020}')
    );
  ...
  it("DeserializeJSON - nested object", async function () {
    ...
    let result: Automobile.Car = dj.deserializeFromJson(
      JSON.parse(
        '{"id":1,"make":"Ford","model":"Escape","year":2020, "owner": { "name": "Paul"}}'
      ),
      Automobile,
      Automobile.Car
    );
