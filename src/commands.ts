...
export async function AddSolutionComponent(
  ...
  let authResponse: PowerAppsConnection = await Authentication.Instance.authenticate(
    AuthParamsPWD
  );

  let response = await fetch(
    `${EnvironmentDetails.org_url}/AddSolutionComponent`,
    {
      method: "POST",
      headers: {
        accept: "application/json",
...
export async function GetSolutionComponentsSummaries(solutionID: string) {
  ...
  let response = await fetch(
    `${EnvironmentDetails.org_url}/msdyn_solutioncomponentsummaries?$filter=(msdyn_solutionid eq ${solutionID})`,
    {
      method: "GET",
      headers: {
        accept: "application/json",
 ...
 */
export async function CompareSolutionSummaries(
  pathA: string,
  pathB: string
): Promise<ISolutionCompareResponse> {
  ...
  let a = scsCollection_A.map((item) => {
    return JSON.stringify(<IMinComponentCompare>{
      componentType: item.msdyn_componenttype,
      displayName: item.msdyn_displayname,
      uniqueName: item.msdyn_name,
      isManaged: item.msdyn_ismanaged,
    });
  });
  let b = scsCollection_B.map((item) => {
    return JSON.stringify(<IMinComponentCompare>{
      componentType: item.msdyn_componenttype,
      displayName: item.msdyn_displayname,
      uniqueName: item.msdyn_name,
  ...
  let respone: ISolutionCompareResponse = {
    isEqual: result,
    uniqueFromPathA: diffA.map((item) => {
      return <IMinComponentCompare>JSON.parse(item);
    }),
    uniqueFromPathB: diffB.map((item) => {
      return <IMinComponentCompare>JSON.parse(item);
    }),
  };
 ...
 */
export async function GetSolutionComponents(
  solutionName
  ...
  let response = await fetch(
    `${EnvironmentDetails.org_url}/solutioncomponents?$filter=solutionid/uniquename eq '${solutionName}'&$expand=solutionid($select=uniquename, version),solutioncomponent_parent_solutioncomponent`,
    {
      method: "GET",
      headers: {
        accept: "application/json",
  ...
  let solutioncomponentCollection: Array<SolutionComponent> = generateComponents(
    solutionComponentResponse
  );

  let index = 0;
  for (var component of solutioncomponentCollection) {
    let response = await getActualComponent(
      component,
    ...
    if (typeof response.name === "string") {
      ...
      solutioncomponentCollection[index].friendlyName =
        response.EntityMetadata.DisplayName.LocalizedLabels[0].Label;
      solutioncomponentCollection[index].logicalName =
        response.EntityMetadata.LogicalName;
    ...
    if (element.componenttype !== ComponentTypes.attribute) {
      ...
      switch (element.componenttype) {
        case ComponentTypes.entity:
          ...
          element.solutioncomponent_parent_solutioncomponent.forEach(
            (element) => {
 ...
 */
async function getActualComponent(
  ...
  if (apiCallType === ComponentTypes[ComponentTypes.entity]) {
    ...
    restRequest = `(EntityFilters=Microsoft.Dynamics.CRM.EntityFilters'Attributes',MetadataId=${component.objectId.replace(
      /{|}/g,
      ""
  ...
  let response = await fetch(
    `${EnvironmentDetails.org_url}/${apiCallType}${restRequest}`,
    {
      method: "GET",
      headers: {
        accept: "application/json",
